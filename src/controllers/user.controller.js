...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // check if user already exist if not show no user found
    // check password if user exists
    // save access token to the user cookies
    ...
    if (!user) {
        throw ApiError(400, "No user found, Create new accout");
    }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const user = await User.findById(decodedToken?._await)
